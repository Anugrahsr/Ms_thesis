...
#It will detect in-frame stop codons, DNA "ambiguity" characters other than N, lack
of stop codon at the end of the sequence and lack of start codon at the beginning of start
codon.
my $orffile = $ARGV[0];
my $counter=0;
my $orfcount=0;
...
if ($sequence{$name} =~ m/ATG(?:[ATGC]{3}(?<!TAG|TAA|TGA)){13,}

(?:TAG|TAA|TGA)$/){$orfcount++;}
else{print "$name\n";}
}
print "$orfcount out of $counter sequences validated as ORFs.\n
# usage perl multifasta.pl ORF.fa|sed 's/\./_/g' > clORF.fa
