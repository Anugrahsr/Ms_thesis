...
tiff("Figure_gc_deviation.tif", width = 22, height = 12, units = 'in', res = 500)
par(mfrow=c(3,3))
for (j in c(1:9)){
Sys.glob(file.path(".", paste(j,"_*.GC_deviation",sep="")))->Files
plot(1, type="n", xlab="", ylab="", xlim=c(0, 1600), ylim=c(-1,
1),main=substring(unlist(strsplit(Files[1],"[.]"))[2], 2))
count<-1
for(i in Files){
...
dev.off()
tiff("Figure_gc_content.tif", width = 22, height = 12, units = 'in', res = 500)
par(mfrow=c(3,3))
for (j in c(1:9)){
